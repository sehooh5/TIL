...
select ename 이름, job 업무, dname 부서이름
from emp, dept
...
select e.ename 직원이름, d.dname 부서명,  to_char(e.sal,'999,999,999')||'원' 월급
from emp e, dept d
where e.deptno = d.deptno
and e.sal >=3000;
...
select e.empno 직원번호
        , e.ename 직원이름
        , e.sal*12 연봉
        ,nvl2(comm, e.sal*12+comm, e.sal*12) 실급여
        , s.grade 급여등급
...
select nvl2(e.ename,e.ename, '미정') 이름, d.deptno 부서번호, d.dname 부서이름 
from emp e, dept d
where e.deptno(+) = d.deptno;
...
select nvl2(e.ename,e.ename,'누구?') 이름 
        ,nvl2(to_char(d.deptno),to_char(d.deptno),'어디?') 부서번호  
        ,nvl2(d.dname,d.dname,'어디?') 부서이름
from emp e, dept d
where e.deptno(+) = d.deptno
union
select nvl2(e.ename,e.ename,'누구?') 이름 
...
select e.empno 사원번호
        , e.ename 사원이름
        , nvl(to_char(m.empno),'없음') 관리자번호
        , nvl(m.ename,'없음') 관리자이름
from emp e, emp m
where e.mgr = m.empno(+);
