...
select e.ename 이름, d.dname 부서명, e.sal 월급
from emp e natural join dept d 
where e.sal >=3000;
...
select e.empno 직원번호, e.ename 직원이름,  e.sal*12 연봉, e.sal*12+comm 실급여, s.grade 급여등급
...
select e.ename 직원이름,  e.sal 월급,  s.grade 급여등급
from emp e join salgrade s on (e.sal between s.losal and s.hisal)
join dept d on (e.deptno = d.deptno)
where upper(d.loc) = 'DALLAS';
...
select e.empno 사원번호, e.ename 사원이름, nvl(to_char(m.mgr),'없음') 관리자번호,nvl (m.ename,'없음') 관리자이름
