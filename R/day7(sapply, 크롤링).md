...
# simple apply = sapply
#첫번째 매개변수에 객체를 주고
#객체의 list 든 vector 든 갯수만큼 
#다음 매개변수인 function 을 실행해 준다
vest_repl <- sapply(doms1,function(x){x$getElementText()}) 
repl_v <- c(repl_v, unlist(vest_repl)) # unlist ->  객체화
...
repeat{
  ...
  # clickElement() 안되면 이렇게 해결!
  remDr$executeScript("arguments[0].click();",pageLink) # 자바스크립트를 사용함, 첫번째 객체 받음 = arguemts[0]
  Sys.sleep(2)
