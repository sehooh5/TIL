...
#scan 은 숫자 읽기에 특화되어있음
nums <- scan("data/sample_num.txt")
word_ansi <- scan("data/sample_ansi.txt",what="")# 문자 읽고싶으면 what=""
words_utf8 <- scan("data/sample_utf8.txt", what="",encoding="UTF-8")
words_utf8_new <- scan("data/sample_utf8.txt", what="")# 깨짐

# readLines 행단위로 읽어서 행단위로 원소에 넣어줌
lines_ansi <- readLines("data/sample_ansi.txt")
lines_ansi <- readLines("data/sample_utf8.txt")# 깨짐, endoing 해줘야함
lines_utf8 <- readLines("data/sample_utf8.txt",encoding="UTF-8")

# read.csv 는 컬럼 구분시 comma
# read.table 은 컬럼 구분시 공백문자 혹은 sep 지정 가능
...
for(num in 1:10){
  num <- as.character(num) # numeric to character= paste(num,"")
